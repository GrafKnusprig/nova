...
test("project layout profile is persisted and validated", async (context) => {
  ...
  assert.equal(view.recency_glow, false);
  assert.equal(view.live_expand, false);
  view.layout_mode = "relations";
  view.layout_compact = true;
  view.recency_glow = true;
  view.live_expand = true;
  validateMap(loaded.document);
  view.recency_glow = "yes";
  assert.throws(() => validateMap(loaded.document), /recency_glow/);
  view.recency_glow = true;
  view.live_expand = "yes";
  assert.throws(() => validateMap(loaded.document), /live_expand/);
  view.live_expand = true;
