...
function validateMap(value: unknown): JsonObject {
  ...
  if (!Array.isArray(value.view.expanded) || !Array.isArray(value.view.hidden_tags) || value.view.hidden_tags.some((tag) => typeof tag !== "string" || !TOKEN_ID.test(tag))) throw new Error("view expansion or tag filters are invalid."); assertObject(value.view.positions, "view.positions"); if (typeof value.view.zoom !== "number" || !Array.isArray(value.view.viewport) || value.view.viewport.length !== 2 || value.view.viewport.some((entry) => typeof entry !== "number")) throw new Error("view geometry is invalid."); if (value.view.workspace !== undefined) assertObject(value.view.workspace, "view.workspace");
...
async function readMap(filePath: string) { const data = validateMap(JSON.parse(await fs.readFile(filePath, "utf8"))); currentMapPath = filePath; lastSerialized = serialized(data); startWatcher(filePath); return { path: filePath, data }; }
...
function registerIpc(): void {
  ...
  ipcMain.handle("mindmap:save-as", async (_event, data: unknown) => { const result = await dialog.showSaveDialog({ title: "Save knowledge-map project", defaultPath: currentMapPath ?? "mindmap.json", filters: [{ name: "Mind-map project", extensions: ["json"] }] }); if (result.canceled || !result.filePath) return undefined; await atomicWrite(result.filePath, data); currentMapPath = result.filePath; startWatcher(currentMapPath); return currentMapPath; });
}
