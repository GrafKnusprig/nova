...
function appTitle(): string { return `NOVA ${VIEWER_VERSION}`; }
...
function createSplashWindow(onShown: (window: BrowserWindow, shownAt: number) => void): BrowserWindow { const window = secureWindow({ title: appTitle(), width: 920, height: 518, frame: false, transparent: false, resizable: false, movable: false, minimizable: false, maximizable: false, fullscreenable: false, alwaysOnTop: true, skipTaskbar: true, show: false }); window.once("ready-to-show", () => { if (window.isDestroyed()) return; window.show(); onShown(window, Date.now()); }); void window.loadURL(`${rendererPageUrl("splash.html")}?version=${encodeURIComponent(VIEWER_VERSION)}`); return window; }
function createMainWindow(splash?: BrowserWindow, splashShownAt = Date.now()): void { const window = secureWindow({ title: appTitle(), minWidth: 900, minHeight: 600, width: 1440, height: 920, show: false }); window.on("page-title-updated", (event) => event.preventDefault()); primaryWindow = window; window.on("closed", () => { if (primaryWindow === window) primaryWindow = undefined; }); window.once("ready-to-show", () => { const reveal = () => { if (!window.isDestroyed()) window.show(); if (splash && !splash.isDestroyed()) splash.close(); }; const remaining = splash ? Math.max(0, 1000 - (Date.now() - splashShownAt)) : 0; setTimeout(reveal, remaining); }); void window.loadURL(rendererUrl()); }
...
async function initAgentsForOpenProject(showConfirmation: boolean): Promise<string> { if (!currentMapPath) throw new Error("Open a project before initializing AGENTS.md."); const destination = await initializeProjectAgents(currentMapPath); if (showConfirmation) { const owner = BrowserWindow.getFocusedWindow() ?? primaryWindow; const options: Electron.MessageBoxOptions = { type: "info", title: "AGENTS.md initialized", message: "Project instructions were initialized.", detail: destination, buttons: ["OK"] }; if (owner) await dialog.showMessageBox(owner, options); else await dialog.showMessageBox(options); } return destination; }
async function initAgentsFromMenu(): Promise<void> { try { await initAgentsForOpenProject(true); } catch (error) { const owner = BrowserWindow.getFocusedWindow() ?? primaryWindow; const options: Electron.MessageBoxOptions = { type: "error", title: "AGENTS.md could not be initialized", message: "Could not copy the NOVA instructions into the project folder.", detail: String(error), buttons: ["OK"] }; if (owner) await dialog.showMessageBox(owner, options); else await dialog.showMessageBox(options); } }
...
function assistantInteractionMode(value: unknown): AssistantInteractionMode { if (value !== "chat" && value !== "note") throw new Error("Invalid assistant interaction mode."); return value; }
function assistantConversationStyle(value: unknown): AssistantConversationStyle { if (value !== "default" && value !== "professional") throw new Error("Invalid assistant conversation style."); return value; }
...
function registerIpc(): void {
  ...
  ipcMain.handle("assistant:chat", (_event, rawMessages: unknown, rawMode: unknown, rawInteractionMode: unknown, rawConversationStyle: unknown) => assistant.chat(chatMessages(rawMessages), assistantMode(rawMode), assistantInteractionMode(rawInteractionMode), assistantConversationStyle(rawConversationStyle)));
